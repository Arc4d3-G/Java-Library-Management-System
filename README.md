...
- This project has multiple exception checks and assertions to ensure books cannot be checked out if the book is not available.
...
2. The user will be prompted to specify whether the search will be by Name or Email. Input either "1" or "2" to proceed.
...
4. If any matches are found, their details will be displayed like Option 5's member table.
5. If no matches are found, a message will be printed stating so.
...
- Once validated, the input is used in the Book() constructor method, which is then returned by the function.
...
- It is designed to be reusable for both displaying All books (Option 2), and displaying search results (Option 3), and prints different messages depending on whether the passed parameter is the libraryBooks list (which contains All books), or the searchResults list created by the searchBooks() method (Option 3).
...
- There are two different search loops in a switch statement for either title or author search.
...
- For a book to be checked out, user is first prompted for a registered member's email (the member that's borrowing the book). If the member is not registered, the process is aborted.
- Next it prompts for the book's ISBN number. Again, if the book does not exists, checkout is aborted.
- At this point, if both the member and book exist, we check if the book is indeed currently available, if not we abort.
...
## Identified flaws (v1.0)
The program in its current state has the following identified flaws in its functionality/logic:
- Books can be added with an availability status of false. Because the only way to change the availability is by returning the book, and a book can only be returned if the member is recorded as being the one that checked it out, it means that there is no way to change a book's availability if it was initially recorded as false.
- There is no way to edit or remove Books/Members after they've been added to the record. A simple typo is now permanently recorded.
- Once a input process has started (such as when adding a new member), there is no way to abort or cancel the process, and in the case of selecting the wrong process, one must proceed with the prompts until the process is complete.
